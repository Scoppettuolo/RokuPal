...
function db_add_column(&$ret, $table, $column, $type, $attributes = array()) {
  ...
  if (array_key_exists('default', $attributes)) {
    ...
    elseif ($attributes['default'] === FALSE) {
      ...
      $default_val = (string) $attributes['default'];
...
function update_batch() {
  ...
  foreach ((isset($_POST['start']) && is_array($_POST['start']) ? $_POST['start'] : array()) as $module => $version) {
...
function update_results_page() {
  ...
  if (!empty($_SESSION['update_results']) && is_array($_SESSION['update_results'])) {
